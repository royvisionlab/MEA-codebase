...
experimentName = '20220426C'; protocolIdx = 4; groupIdx = 1:2;
...
for gg = 1 : length(groupIdx)
    ...
    for bb = 1 : length(block)
        ...
        if blockCount == 0 % Initialize
            sY = zeros([length(obj.spikeTimes),numYChecks,numXChecks,staFrames]);
            sB = zeros([length(obj.spikeTimes),numYChecks,numXChecks,staFrames]);
        end
...
%%
outdir = '/Users/michaelmanookin/Documents/Data/rawdata/MEA_Data/Analysis/';

expName = strsplit(obj.dataDir,'/');
expName = expName{end-1};

cluster_id = obj.spikeIdentity;

sB = sB / max(abs(sB),[],'all');
sY = sY / max(abs(sY),[],'all');
sB = 0.5*sB + 0.5;
sY = 0.5*sY + 0.5;

sta = zeros([size(sY),3]);
sta(:,:,:,:,1:2) = repmat(sY,[1,1,1,1,2]);
sta(:,:,:,:,3) = sB;

sB = 2*sB - 1;
sY = 2*sY - 1;

save([outdir,expName,'_sta.mat'],'-v7.3','sta','cluster_id');
...
for jj = 1 : size(sY,1)
    ...
    title(['Yellow Rf; Cluster: ',num2str(obj.spikeIdentity(jj))]);
    ...
    title(['Cluster: ',num2str(obj.spikeIdentity(jj))]);
    
    if jj == 1
        print(f, [outdir,expName,'_STA'], '-dpsc');
    else
        print(f, [outdir,expName,'_STA'], '-dpsc', '-append');
    end
end
