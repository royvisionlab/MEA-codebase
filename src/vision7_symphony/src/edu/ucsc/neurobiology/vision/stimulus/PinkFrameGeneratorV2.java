 ...
 * In version 2, I changed the random number generator to the Mersenne Twister,
 * which has a much longer period and better statistical properties.  I also am now
 * drawing the initial gaussian values directly from the random number generator,
 * instead of casting them as shorts before using them.
 * This stimulus is not used in modern experiments, but it still functions,
 * and can be used to analyze old data.
 *
 * @author Matthew Grivich, University of California, Santa Cruz
 */
public class PinkFrameGeneratorV2
    extends FrameGenerator {

    private RandomMersenne mersenne;
    ...
    private double normalizer; //used to normalize the frames to the correct standard deviation
    private double[] currentColors; //current frame
    ...
    public PinkFrameGeneratorV2(int width, int height, int spatialFreqDepth,
                                int temporalFreqDepth, float[] sigma, int startSeed,
                                int colorType, MovieType noiseType) {
        ...
        currentColors = new double[width * height * 3];
...
//                        color[i] = 2*(currentColors[i]/normalizer)*(currentColors[i]/normalizer )+.5f;
                    color[i] = (float) ( (currentColors[i] / normalizer) + .5);
...
//                       frameNumber++;
    ...
    private float[] addOneSpatialFreq(float[] colors, int depth) {
        ...
        for (int i = 0, index = 0; i < currentNPixels; i++) {
            if (colorType == RGB) {
                for (int c = 0; c < 3; c++) {
                    tempColors[index++] = (float) mersenne.nextGaussian(0.0, sigma[0]);
                }
            } else {
                float val = (float) mersenne.nextGaussian(0.0, sigma[0]);
                for (int c = 0; c < 3; c++) {
                    tempColors[index++] = val;
                }
        ...
        if (depth != 0) {
            ...
            horPhase = (Math.abs(mersenne.nextInt())) % pixelSize;
            vertPhase = (Math.abs(mersenne.nextInt())) % pixelSize;
