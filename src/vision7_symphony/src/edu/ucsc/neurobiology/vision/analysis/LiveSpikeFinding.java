...
extends AbstractCalculation {
    ...
    final int SPIKE_BUFFER_SIZE = 60;


    public LiveSpikeFinding() {
        if (Vision.isGUIBased()) {
            diagnosticPanel = new JPanel(new TableLayout(0, 0));
        } else {
            throw new IllegalStateException("Live Spike Finding requires a GUI.");
        }
    }
    ...
    public void setParameters(HashMap<String, String> parameters) throws Exception {
        ...
        sampleInputStream = new MultipleCompressedSampleInputStream(
                rawDataSource, bufferSizeInBytes, nBuffers);
        RawDataHeader header = sampleInputStream.getHeader();
        ...
        if (saveRawData) {
            ...
            int secondsToStream = Integer.parseInt(p.get(
            "saveRawData.Stop Streaming After (sec)"));
            ...
            if (header.getDatasetIdentifier().equals("9999-99-99-data999")) {
                System.out.println("WILL NOT SAVE/STREAM DATASET " +
