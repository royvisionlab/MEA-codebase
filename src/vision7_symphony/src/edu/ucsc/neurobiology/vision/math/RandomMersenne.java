...
/*
 ...
 This code was copied out from COLT on 2004/07/23 by Matthew Grivich, to prevent version
 conflict problems in the future, and renamed to RandomMersenne.  The classes nextDouble,
 nextPhase, and main were added.
 */
...
public class RandomMersenne {
    ...
    // cache for Box-Mueller algorithm to generate Gaussian numbers
     ...
     * <tt>[Integer.MIN_VALUE, Integer.MAX_VALUE]</tt> (including <tt>Integer.MIN_VALUE</tt>
     * and <tt>Integer.MAX_VALUE</tt>).
     */
    public int nextInt() {
    ...
    /*
       returns a random number between [0.0, 1.0). [ means inclusive.  ( means exclusive
       There are 2^-53 possible values, which is the maximum possible for a double
    ...
    /*
      returns a random number on the double open interval (0.0, 1.0).
      There are 2^32 possible values, which is the precision of nextInt
     */
    public double nextDouble32Open() {
        return
            ( ( (double) nextInt()) + 2147483648.5) * (1.0 / 4294967296.0);

    }


    /* returns a random number between [-Math.PI, Math.PI)
      It keeps the 2^32 possible values from nextInt.  More precision is possible,
      using code similar to that of nextDouble, but that is unnecessary, and takes
      twice as many random numbers.
     */
    public double nextPhase() {
        return (nextInt()) * 1.4629180792671596E-9;
    ...
    public double nextGaussian(double mean, double standardDeviation) {
        ...
        double x, y, r, z;
        do {
            x = 2.0 * nextDouble32Open() - 1.0;
            y = 2.0 * nextDouble32Open() - 1.0;
            r = x * x + y * y;
        } while (r >= 1.0);
